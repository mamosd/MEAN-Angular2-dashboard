"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var router_1 = require('@angular/router');
var http_1 = require('@angular/http');
require('rxjs/add/operator/toPromise');
var ProfilesComponent = (function () {
    function ProfilesComponent(router, http) {
        this.router = router;
        this.http = http;
        this.rowsOnPage = 10;
        this.profiles = [];
        this.profilesurl = "api/profiles/";
        this.name = 'Angular';
        this.title = 'Profile Labels';
    }
    ProfilesComponent.prototype.ngOnInit = function () {
        this.profiles = [];
        this.getProfiles();
    };
    ProfilesComponent.prototype.getProfiles = function () {
        var _this = this;
        this.http.get(this.profilesurl)
            .map(function (res) { return res.json(); })
            .subscribe(function (profiles) { _this.profiles = profiles; });
    };
    ProfilesComponent.prototype.gotoAddProfile = function () {
        this.router.navigate(['/addprofile']);
    };
    ProfilesComponent.prototype.deleteProfile = function (i) {
        var _this = this;
        this.http.delete(this.profilesurl + this.profiles[i]._id)
            .map(function (res) { return res.json(); })
            .subscribe(function (x) { _this.profiles.splice(i, 1); console.log(_this.profiles); });
        ;
    };
    ProfilesComponent.prototype.editProfile = function (i) {
        this.router.navigate(['/editprofile/', this.profiles[i]._id]);
    };
    ProfilesComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'my-profiles',
            templateUrl: 'profiles.component.html',
        }), 
        __metadata('design:paramtypes', [router_1.Router, http_1.Http])
    ], ProfilesComponent);
    return ProfilesComponent;
}());
exports.ProfilesComponent = ProfilesComponent;
//# sourceMappingURL=profiles.component.js.map